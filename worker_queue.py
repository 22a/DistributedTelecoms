...
from server_test import Worker
from errors import WorkerUnavailableException
from thread_comm import ThreadComm
from server_test import Worker
...
class WorkerHeap:
	...
	def just_inserted(self):
		return self.just_inserted
	...
	def insert( self, worker ):
		#print( "type(worker): ", type(worker) )
		#print( "type(Worker): ", type( Worker ) )
		#print( type( worker ) is Worker )
		#print( type( worker ))
		...
		if ( self.fixed_size == False ):
			...
			if ( self.N > 1 ):
				...
				worker.set_index_in_heap( 1 ) # temporarily disabled
				self.heap[ self.N ] = dict([( self.generate_key( worker ), worker )] )

			self.just_inserted = worker
			return True
	...
	def generate_key( self, worker ):
		...
		return worker.get_items_assigned() / worker.get_items_processed()
